...
impl RenderTarget {
    ...
    pub fn begin_frame(&mut self) {
        if self.surface_texture.is_none() {
            ...
            self.color_view = Some(self.surface_texture.as_ref().unwrap().texture.create_view(
            ...
            self.depth_view = Some(
                texture::TextureWrapper::create_depth_texture(
                    &self.device,
                    &self.config,
                    "Depth Texture",
                )
